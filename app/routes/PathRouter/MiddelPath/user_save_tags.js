...
router.get('/', ifNotLoggedin, async (req, res, next) => {
    try {
        ...
        dbConnection.execute("SELECT * FROM `news` LEFT JOIN group_section ON group_section.news_id = news.news_id LEFT JOIN section ON section.section_id = group_section.section_id LEFT JOIN save_section ON save_section.tags_id = group_section.section_id LEFT JOIN users ON users.id = news.user_id WHERE save_section.t_user_id = ? ORDER BY news.news_id DESC;", [req.session.userID])
            .then(([rows]) => {
