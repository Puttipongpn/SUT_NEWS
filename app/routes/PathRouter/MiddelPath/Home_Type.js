...
router.get('/news/insut', ifNotLoggedin, async (req, res, next) => {
    try {
        ...
        //const attention = await dbConnection.execute("SELECT users.* , news.* FROM news LEFT JOIN users ON news.user_id = users.id LEFT JOIN approve_news ON approve_news.news_id = news.news_id WHERE approve_news.status_id = 2 AND ( news.end IS NULL OR news.end < DATE_SUB(NOW(), INTERVAL 7 DAY)) ORDER BY news.view_count DESC LIMIT 10 ");
        //SQL โชวข่าวเรียงจากที่มีจำนวนเข้าชมเยอะสุด และยังไม่สิ้นสุดเวลากิจกรรม หรือข่าวที่ไม่ได้ใส่วันสิ้นสุดกิจกรรม จะหายไปเมื่อครบ 7 วัน
        const attention = await dbConnection.execute("SELECT news.*, users.*, COUNT(`like`.like_news_id) AS like_count FROM news LEFT JOIN users ON news.user_id = users.id LEFT JOIN approve_news ON approve_news.news_id = news.news_id LEFT JOIN `like` ON `like`.like_news_id = news.news_id WHERE approve_news.status_id = 2 AND news.news_type_id = 1 AND ((news.end IS NULL AND news.time_stamp > DATE_SUB(NOW(), INTERVAL 7 DAY)) OR (news.end > NOW() AND news.time_stamp > DATE_SUB(NOW(), INTERVAL 7 DAY)))GROUP BY news.news_id ORDER BY like_count DESC, news.view_count DESC LIMIT 10;");
...
router.get('/news/outsut', ifNotLoggedin, async (req, res, next) => {
    try {
        ...
        //const attention = await dbConnection.execute("SELECT users.* , news.* FROM news LEFT JOIN users ON news.user_id = users.id LEFT JOIN approve_news ON approve_news.news_id = news.news_id WHERE approve_news.status_id = 2 AND ( news.end IS NULL OR news.end < DATE_SUB(NOW(), INTERVAL 7 DAY)) ORDER BY news.view_count DESC LIMIT 10 ");
        //SQL โชวข่าวเรียงจากที่มีจำนวนเข้าชมเยอะสุด และยังไม่สิ้นสุดเวลากิจกรรม หรือข่าวที่ไม่ได้ใส่วันสิ้นสุดกิจกรรม จะหายไปเมื่อครบ 7 วัน
        const attention = await dbConnection.execute("SELECT news.*, users.*, COUNT(`like`.like_news_id) AS like_count FROM news LEFT JOIN users ON news.user_id = users.id LEFT JOIN approve_news ON approve_news.news_id = news.news_id LEFT JOIN `like` ON `like`.like_news_id = news.news_id WHERE approve_news.status_id = 2 AND news.news_type_id = 2 AND ((news.end IS NULL AND news.time_stamp > DATE_SUB(NOW(), INTERVAL 7 DAY)) OR (news.end > NOW() AND news.time_stamp > DATE_SUB(NOW(), INTERVAL 7 DAY)))GROUP BY news.news_id ORDER BY like_count DESC, news.view_count DESC LIMIT 10;");
